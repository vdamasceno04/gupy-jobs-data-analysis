...
def setHowManyCompanies(filepath):
...
def setJobsPerCompany(filepath):
...
def getJobsInACity(filepath, city):
    locations = csvhandle.getLocation(filepath)

    handled = city.replace(" " , "").lower()
    k = 0
    for i in locations:
...
def setJobsInACity(filepath, city):
